...
fun SearchBox(
    ...
    val state = remember { locationSearchState }
    ...
    Column(
        ...
        AnimatedVisibility(visible = locationDetails.isNotEmpty()) {
            LazyColumn(
                horizontalAlignment = Alignment.CenterHorizontally
            ) {
                items(state.locationDetails.size) { index ->
                    Box(modifier = Modifier.clickable {
                        onLocationClick(locationDetails[index])
                    }
                    ) {
                        DimText(text = locationDetails[index].cityName)
                    }
                }
            }
        }
